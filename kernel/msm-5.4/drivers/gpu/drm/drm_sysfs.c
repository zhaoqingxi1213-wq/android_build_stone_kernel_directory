...
extern long dsi_display_mipi_reg_ead(struct drm_connector *connector,
                                        char *buf);
...
{
	struct drm_connector *connector = to_drm_connector(device);
	/* return dsi_display_mipi_reg_ead(connector, buf); */
	return 0;

}
extern long dsi_display_mipi_reg_rite(struct drm_connector *connector,
...
{
	struct drm_connector *connector = to_drm_connector(device);
	/* return dsi_display_mipi_reg_rite(connector, (char *)buf, count); */
	return 0;
}
...
extern long dsi_display_get_hb_status(struct drm_connector *connector);
				...
				char *buf)
{
	int hbm_status;
	struct drm_connector *connector = to_drm_connector(device);
	/* hbm_status = dsi_display_get_hb_status(connector); */
	return sprintf(buf, "%u\n", hbm_status);
}
extern long dsi_display_set_bm(struct drm_connector *connector,
                        int hbm_status);
				...
				const char *buf, size_t count)
{
	...
	if (ret)
		return ret;
	/* ret = dsi_display_set_bm(connector, hbm_status); */
	return ret ? ret : count;
}
/* end modify */

/* BSP.LCM - 2022.07.13 - modify for LCM add doze_brightness */
extern long dsi_display_get_doze_brightnes(struct drm_connector *connector);
				...
				char *buf)
{
	int doze_brightness;
	struct drm_connector *connector = to_drm_connector(device);
	/* doze_brightness = dsi_display_get_doze_brightnes(connector); */
	return sprintf(buf, "%u\n", doze_brightness);
}
extern long dsi_display_set_doze_brightnss(struct drm_connector *connector,
                        int doze_brightness);
				...
				const char *buf, size_t count)
{
	...
	if (ret)
		return ret;
	/* ret = dsi_display_set_doze_brightnss(connector, doze_brightness); */
	
	return ret ? ret : count;
}
extern long dsi_display_get_flat_moe(struct drm_connector *connector);
				...
				char *buf)
{
	int flat_mode;
	struct drm_connector *connector = to_drm_connector(device);
	/* flat_mode = dsi_display_get_flat_moe(connector); */
	return sprintf(buf, "%u\n", flat_mode);
}
extern long dsi_display_set_lat_mode(struct drm_connector *connector,
                        int flat_mode);
				...
				const char *buf, size_t count)
{
	...
	if (ret)
		return ret;
	/* ret = dsi_display_set_lat_mode(connector, flat_mode); */
	return ret ? ret : count;
